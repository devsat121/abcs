...
class MainActivity : AppCompatActivity() {
    ...
    private val helperStatusReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            updateHelperBadge(intent.getBooleanExtra("connected", false))
        }
    }
    ...
    private fun setupRecyclerView() {
        adapter = RecordingAdapter(
            ...
            onStarClick = { recording -> viewModel.toggleStarred(recording) }
        )
    ...
    private fun observeViewModel() {
        ...
        viewModel.isRecording.observe(this) { updateRecordingBadge(it) }

        lifecycleScope.launch {
            viewModel.selectedIds.collectLatest { sel ->
                adapter.setSelectedIds(sel)
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Record") { _, _ ->
                ...
                CallRecordingService.startRecording(
                    this, num, com.cloudacr.app.data.Recording.CallType.UNKNOWN
                )
            }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.action_clear_selection -> { viewModel.clearSelection(); true }
