...
class RecordingDetailActivity : AppCompatActivity() {
    ...
    private fun loadRecording(id: Long) {
        lifecycleScope.launch {
            val rec = (application as CloudACRApp).repository.getById(id) ?: run {
    ...
    private fun displayRecording(rec: Recording) {
        ...
        binding.ivStar.setOnClickListener {
            lifecycleScope.launch {
                ...
                val newStarred = !(recording?.isStarred ?: false)
                repo.setStarred(rec.id, newStarred)
                recording = recording?.copy(isStarred = newStarred)
                binding.ivStar.setImageResource(
                    if (newStarred) R.drawable.ic_star_filled else R.drawable.ic_star_outline
                )
            }
        }
    ...
    private fun setupSaveNotesButton(rec: Recording) {
        binding.btnSaveNotes.setOnClickListener {
            ...
            lifecycleScope.launch {
                (application as CloudACRApp).repository.updateNotes(rec.id, notes.ifBlank { null })
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.action_share -> { shareRecording(); true }
    ...
    private fun shareRecording() {
        ...
        // FIX #3 dependency: FileProvider now declared in manifest + file_provider_paths.xml
